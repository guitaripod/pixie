...
pub async fn handle_edit(mut req: Request, ctx: RouteContext<()>) -> Result<Response> {
    ...
    // TODO: Implement multipart/form-data support for Cloudflare Workers
    AppError::BadRequest(
        "Image editing is temporarily unavailable. The backend needs to be updated to support multipart/form-data requests to OpenAI. This requires a different approach in Cloudflare Workers environment.".to_string()
    ).to_response()
}